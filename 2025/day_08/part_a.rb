...
# puts distances.first(10).map(&:to_s)

SHORTEST_DISTANCE_COUNT = 1000

circuits = []
distances.first(SHORTEST_DISTANCE_COUNT).each do |distance|
...
puts circuits.map(&:junction_boxes).map(&:count).uniq.sort.last(3).inject(&:*)
